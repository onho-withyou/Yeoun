...
function initEmpGrid() {
  empGrid = new tui.Grid({
    ...
    columns: [
      ...
      { header: '직급서열',     name: 'rankOrder',  hidden: true,  sortable: true },
  ...
  // 🔥 직급 헤더 클릭하면 rankOrder 기준으로 정렬 (우리가 직접 정렬)
  empGrid.on('beforeSort', ev => {
    ...
    console.log('[empList] 직급 정렬 실행, asc =', posSortAsc);
