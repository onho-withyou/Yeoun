...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
	...
	@Query(value = """
		    SELECT
		        oi.PRODUCT_ID AS prdId,
		        p.PRD_NAME AS productName,
		        SUM(oi.ORDER_QTY) AS totalOrderQty
		    FROM ORDER_ITEM oi
		    JOIN PRODUCT_MST p ON p.PRD_ID = oi.PRODUCT_ID
		    JOIN ORDERS o ON o.ORDER_ID = oi.ORDER_ID
		    WHERE o.ORDER_STATUS = 'CONFIRMED'
		      AND (:group IS NULL OR p.ITEM_NAME = :group)
		    GROUP BY oi.PRODUCT_ID, p.PRD_NAME
		""", nativeQuery = true)
